...
namespace UVR_SLAM {
	...
	public:
		...
		Frame* GetReverseGraphFrame(int idx);
		bool isAddedGraph();
		void UpdateGraphConnection();//Frame* pTargetKF
		...
		std::set<Frame*> GetGraphFramesSet();
	...
	private:
		...
		bool mbUpdateGraph;
		Frame* mpNewGraphKF;
		std::mutex mMutexWindowFrames, mMutexGraphFrames;
