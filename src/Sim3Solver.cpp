...
namespace UVR_SLAM {
	...
	Sim3Solver::Sim3Solver(Frame* pKF1, Frame* pKF2, const std::vector<MapPoint*> &vpMatched12, const bool bFixScale)
	...
	cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore, std::vector<bool> &vbInliers, int &nInliers)
	{
		...
		while (mnIterations<mRansacMaxIts && nCurrentIterations<nIterations)
		{
			...
			std::mt19937 g(rd());
			std::shuffle(vAvailableIndices.begin(), vAvailableIndices.end(), g);

			// Get min set of points
			for (short i = 0; i < 3; ++i)
			{
				
				int randi = vAvailableIndices[i];
