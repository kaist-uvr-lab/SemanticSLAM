...
UVR_SLAM::Initializer::Initializer(cv::Mat _K):mbInit(false), mpInitFrame1(nullptr), mpInitFrame2(nullptr), mpTempFrame(nullptr) {
	//mK.convertTo(mK, CV_64FC1);
}
...
bool UVR_SLAM::Initializer::Initialize(Frame* pFrame, bool& bReset, int w, int h) {
	...
	if (!mpInitFrame1) {
		...
		float score;
		////E  찾기 : OpenCV
		cv::Mat E12 = cv::findEssentialMat(vTempPts1, vTempPts2, mpInitFrame1->mK, cv::FM_RANSAC, 0.999, 1.0, vFInliers);
		////E  찾기 : OpenCV
		//////F, E를 통한 매칭 결과 반영
		...
		cv::Mat Map3D;
		cv::Mat K;
		mpInitFrame1->mK.convertTo(K, CV_64FC1);
		...
		//////////////////////////////////////
		cv::Point2f ptBottom(0, mpInitFrame1->mnHeight);
		...
		for (int i = 0; i < matTriangulateInliers.rows; i++) {
			...
			proj1 = mpInitFrame1->mK*proj1;
			proj2 = mpInitFrame2->mK*proj2;
