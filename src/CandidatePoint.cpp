...
namespace  UVR_SLAM{
	CandidatePoint::CandidatePoint():octave(0), bCreated(false), mbDelete(false), mnFail(0), mnSuccess(0), mnTotal(0), mnMapPoint(0), mnCandidatePointID(++nCandidatePointID), mnLastFrameID(-1), mnVisibleFrameID(-1), mbLowQuality(true),
mbOptimized(false)
	{
		mpMapPoint = nullptr;
	}
	CandidatePoint::CandidatePoint(MatchInfo* pRefKF, int alabel, int aoct):mpRefKF(pRefKF), label(alabel), octave(aoct), bCreated(false), mbDelete(false), mnFail(0), mnSuccess(0), mnTotal(0), mnMapPoint(0),
mnCandidatePointID(++nCandidatePointID), mnLastFrameID(-1), mnVisibleFrameID(-1), mbLowQuality(true), mbOptimized(false)
	{
		mpMapPoint = nullptr;
	...
	void CandidatePoint::ComputeQuality(){
		...
		/*if (nLastFrame == -1)
		{
			...
			int nFrame = nLastFrame + 50;
			float ratio = ((float)nS) / (nS + nF);
			if (nFrame < nCurrFrame) {
				b = false;
			}
			else if (ratio < 0.3f){
				b = false;
